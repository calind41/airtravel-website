...
import UserDataFormPopup from "./UserDataFormPopup/UserDataFormPopup";
...
export default function UserPersonalArea({ t }) {
  ...
  useEffect(() => {
    ...
    return () => {
      document.querySelectorAll(`.${styles.tabs} > div`).forEach((elem) => {
        elem.removeEventListener("click", (evt) => chooseTab(evt));
      });
    };
  }, [hiddenTabs]);
  ...
  return (
    <div className={styles.container}>
      ...
      <UserDataFormPopup t={t} />
      {hiddenTabs ? null : (
        <div className={styles.tabsWrapper}>
          <div className={styles.tabs}>
            <div
              id='t1'
              className={`${styles.myTickets} ${styles.selectedTab}`}
            >
      ...
      {showMyTickets && <UserTickets t={t} toggleTabs={toggleTabs} />}
      {showPersonalData && <UserPersonalData t={t} />}
      {showSettings && <UserSettings t={t} />}
