...
import { ticketProps } from "../../FlightResults/FlightResults";
import {
  baggageIcon,
  baggageIconSmall,
...
export default function TicketModal({ unmountModal, from, to, date }) {
  const closeModal = (evt) => {
    const hasClassContainer = evt.target.classList.contains(styles.container);
    const hasClassCloseIcon = evt.target.classList.contains(styles.closeIcon);
    if (hasClassContainer) {
      dom("html").style.overflow = "visible";
      dom("html").style.height = "auto";
      evt.target.classList.add(styles.fadeOut);
      setTimeout(() => {
        unmountModal();
      }, 230);
    }
    if (hasClassCloseIcon) {
      dom("html").style.overflow = "visible";
      dom("html").style.height = "auto";
      dom(`.${styles.container}`).classList.add(styles.fadeOut);
      setTimeout(() => {
        unmountModal();
      }, 230);
    }
...
function Booking({ setWithoutBaggage }) {
  const selectThisButton = (className) => {
    if (className === "withBaggageBtn") {
      ...
      dom(`.${styles.withoutBaggageBtn}`).classList.remove(styles.selectedBtn);
    } else {
      setWithoutBaggage(true);
      dom(`.${styles.withBaggageBtn}`).classList.remove(styles.selectedBtn);
    }
    dom(`.${styles[className]}`).classList.add(styles.selectedBtn);
  };
  return (
