...
export default function Departure({ id, cancelFilters, setNumOfFilters }) {
  ...
  useEffect(() => {
    const closeAllDropdowns = (evt) => {
      const departureTimesDropdown = document.querySelector(
        `#departureTimesDropdown_${id}`
      );
      ...
      if (
        ...
        document
          .querySelector(`#timeSelect_${id} .${styles.arrowIcon} svg`)
  ...
  const cancelDepartureSettingsOnMouseEnterHandler = () => {
    const floatNotif = document.querySelector(`#floatingNotification2_${id}`);
  ...
  const cancelDepartureFilters = () => {
    ...
    const floatingNotification = document.querySelector(
      `#floatingNotification2_${id}`
    );
    const cancelFilterIcon = document.querySelector(`#cancelFilterIcon2_${id}`);
  ...
  const toggleDepartureTimesDropdown = () => {
    document
      .querySelector(`#departureTimesDropdown_${id}`)
  ...
  const displayDepartureCancelIcon = () => {
    ...
    document.querySelector(`#cancelFilterIcon2_${id}`).style.display = "block";
    floatingNotification.classList.remove(styles.floatingNotificationDown);
    floatingNotification.classList.remove(styles.floatingNotificationUp);
  ...
  const toggleAirportDropdown = () => {
    document
      .querySelector(`#airportDropdwon_${id}`)
      .classList.toggle(styles.displayFlex);
    document
      .querySelector(`#departureAirport_${id} .${styles.arrowIcon} svg`)
      .classList.toggle(styles.rotate180deg);
  };
  return (
