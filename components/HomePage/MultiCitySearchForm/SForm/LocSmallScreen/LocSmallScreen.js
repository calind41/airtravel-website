...
import styles from "./LocSmallScreen.module.css";
import { locations } from "../FInput/FInput";

export default function LocSmallScreen({ locType, nr, passProps, passInput }) {
  ...
  const handleClick = (loc) => {
    ...
    if (localStorage.getItem("inType") === "From") {
      document.querySelector(`#fromInputId${nr}`).value = text;
      document.querySelector(`#fromInputAcronymId${nr}`).textContent =
        loc.acronym;
      document.querySelector(`#fromInputId${nr}`).blur();
      // passProps();
    }
    // inputType `to`
    else {
      document.querySelector(`#toInputId${nr}`).value = text;
      document.querySelector(`#toInputAcronymId${nr}`).textContent =
        loc.acronym;
      document.querySelector(`#toInputId${nr}`).blur();
    }
    // Close locations
    document
      .querySelectorAll(`#locationSmallScreenId${nr}`)
  ...
  return (
    <div id={`locationSmallScreenId${nr}`} className={styles.container}>
      <div>
        <span>{locType}</span>{" "}
        <svg
