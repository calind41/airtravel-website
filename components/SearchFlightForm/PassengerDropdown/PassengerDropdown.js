...
import stylesCalendarC from "../FlightDate/CalendarC/CalendarC.module.scss";

import Dropdown from "./Dropdown/Dropdown";
import stylesDropdownC from "./Dropdown/Dropdown.module.scss";
...
export default function PassengerDropdown({
  ...
  const [selectedType, setSelectedType] = useState(t("flightType-economy"));
  ...
  const handleClick = (evt) => {
    ...
    while (
      ...
      if (node.classList.contains(stylesDropdownC.dropdownC)) {
        return;
      }
  ...
  return (
    <div
      onClick={handleClick}
      className={`${styles.passengerC} ${styles.pC} ${styles[passengerCMultiCityClass]} ${styles[positionClass]} ${styles[leftPosClass]} ${styles[widthClass]}`}
    >
      {/* <span className={styles.passengerPlaceholder}>Количество</span> */}
      <span className={styles.passengerPlaceholder}>
        {t("PassengerDropdown-passengerPlaceholder")}
      </span>
      ...
      <div>
        ...
        <span>
          {nrAdults === 1
            ? t("PassengerDropdwon-passengerTextSingle")
            : t("PassengerDropdwon-passengerTextMultiple")}
        </span>
