...
export default function CalendarC({ passDepartingValues, t }) {
  const [tripType, setTripType] = useState(t("TripButtons-oneWayTripBtn"));
  ...
  useEffect(() => {
    ...
    localStorage.setItem("tripType", t("TripButtons-oneWayTripBtn"));
  ...
  const handleScroll = (evt) => {
    ...
    let idx = breakPoints.findIndex((number) => {
  ...
  const receiveSelectedDate = (date, mNr) => {
    ...
    if (tripType === t("TripButtons-oneWayTripBtn")) {
  ...
  return (
    <div id='wrapperId' className={styles.wrapper}>
      ...
      <TripButtons t={t} passTripType={receiveTripType} selected={tripType} />
      <DaysOfWeek />
