...
export default function SortByModal({ unmountModal }) {
  const { state } = useContext(FiltersStateContext);
  const { dispatch } = useContext(FiltersDispatchContext);
  const { initialAscendingSort, initialSortByCriteria } = state;

  const [ascendingSort, setAscendingSort] = useState(initialAscendingSort);
  const [criteria, setCriteria] = useState(initialSortByCriteria);
