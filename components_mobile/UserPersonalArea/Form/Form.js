...
export default function Form({
  ...
  const saveUserData = () => {
    if (isUpdated && !additionalPassenger) {
      // alert("updated");
      const updating = true;
      console.log("is updated and not aditional passenger");
      transmitPassengerData(
        {
          fName: firstName,
          lName: lastName,
          ...
          serialNrD: serialNr,
          serialNrReleaseDateD: serialNrReleaseDate,
          serialNrExpireDateD: serialNrExpireDate,
          nationalityD: nationality,
          genderD: gender,
          id,
        },
        updating
      ...
      <div className={` ${stylesPopup.inputs} ${stylesUPD.inputs}`}>
        ...
        <div onClick={saveUserData} className={btnClassNames}>
          {update
            ...
            : getLanguageSpecificContent("UserDataFormPopup-placeholder10")}
        </div>
      </div>
    </form>
  );
